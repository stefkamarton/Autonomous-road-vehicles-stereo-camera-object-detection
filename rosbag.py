...
def parse_args():
  ...
  parser.add_argument('--load_dir', dest='load_dir', help='directory to load models', default="models_stereo", type=str)
...
if __name__ == '__main__':
  ...
  with torch.no_grad():
    ...
    info_dict = yaml.load(subprocess.Popen(['rosbag', 'info', '--yaml', args.inputbagfile], stdout=subprocess.PIPE).communicate()[0])
    pp = pprint.PrettyPrinter(indent=4)
    pp.pprint(info_dict)
    bag = rosbag.Bag(args.inputbagfile)
    genBag = bag.read_messages(args.readTopic)
    for k, b in enumerate(genBag):
      print("OK, %d / %d" % (k, info_dict['messages']))
      cb = CvBridge()
      cv_image = cb.imgmsg_to_cv2(b.message, b.message.encoding)
      cv_image_r = cv2.resize(cv_image, dsize=(0, 0), fx=1.0, fy=1.0)
      width = cv_image_r.shape[1]
      half_width = width // 2
      cv_image_left = cv_image_r[:, :half_width]
      cv_image_right = cv_image_r[:, half_width:]
      cv2.imwrite(args.outputdir + '/' +  '_r.jpg', cv_image_right)
      cv2.imwrite(args.outputdir + '/' + '_l.jpg', cv_image_left)
      # read data
      img_l_path = args.outputdir +'/_l.jpg'
      img_r_path = args.outputdir +'/_r.jpg'
      ...
      calib = kitti_utils.read_obj_calibration(args.camcalib)
